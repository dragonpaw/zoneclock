    ...
    def __init__(self, datetime, hours_ahead, utc_offset):
        ...
        for _ in range(5):  # Retries
            try:
                ...
                self.midnight = time.mktime(parse_time(moon_data["moonphase"]["time"]))
                if "moonrise" in moon_data:
                    self.rise = time.mktime(parse_time(moon_data["moonrise"]["time"]))
                else:
                    self.rise = None
                if "moonset" in moon_data:
                    self.set = time.mktime(parse_time(moon_data["moonset"]["time"]))
                else:
                    self.set = None
                return  # Success!
            except:
...
try:
    ...
    LATITUDE, LONGITUDE = NETWORK.fetch_data(
        ...
        json_path=[["geoplugin_latitude"], ["geoplugin_longitude"]],
    )
...
while True:
    ...
    GROUP[8].color = GROUP[9].color = 0x00FF00 if EVENT_TIME.tm_hour < 12 else 0xC04000
